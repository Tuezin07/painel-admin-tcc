...
const db = require('./db'); // Certifique-se que db.js está usando variáveis de ambiente
...
app.use(express.static(path.join(__dirname, 'painel-admin', 'public')));

// Página de login
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'painel-admin', 'public', 'login.html'));
});
  ...
  db.query(sql, params, (err, results) => {
    ...
    <head>
      ...
      <style>
        ...
        h1 { text-align:center; padding-top:20px; color:#2c3e50; }
        form { text-align:center; margin-bottom:20px; }
        select { padding:10px; font-size:16px; border-radius:8px; border:1px solid #ccc; margin-right:10px; background-color:#fff; }
        button { padding:10px 20px; font-size:16px; background-color:#3498db; border:none; color:white; border-radius:8px; cursor:pointer; transition:0.3s ease; display:inline-block; margin-top:10px; width:auto; }
        button:hover { background-color:#2980b9; }
        table { width:90%; margin:auto; border-collapse:collapse; box-shadow:0 0 10px rgba(0,0,0,0.1); background:white; border-radius:12px; overflow:hidden; }
        ...
        .top-buttons { display:flex; justify-content:center; margin-top:20px; margin-bottom:20px; }
        .button { padding:10px 20px; font-size:16px; background-color:#3498db; border:none; color:white; border-radius:8px; cursor:pointer; transition:0.3s ease; text-decoration:none; }
        .button:hover { background-color:#2980b9; }
      </style>
    </head>
    <body>
